    ...
    sections: [
      ...
      },
      {
        ...
        body: [
          ...
          '测试环境完整工具地址为 https://mcp-test.spdex.com/mcp?tool_subset=spdex-full。使用该地址并重新连接后，客户端应发现 35 个工具；spdex-basic 会显示 9 个基础工具。',
        ],
      },
      {
        heading: '获取访问凭证',
        body: [
          '个人用户登录 NewSpdex 后，进入会员中心的“AI 与 MCP 连接”，点击“新建”。完整使用 SPdex MCP 时选择“完整 35 工具”；页面会授权当前开放工具所需的 20 项 scope。创建后立即把只完整显示一次的 token 保存到客户端安全配置中。',
        ...
        steps: [
          ...
          '新建连接，填写可识别的客户端名称和用途。',
          '完整使用时选择“完整 35 工具”；轻量查询才选择“基础 9 工具”。',
      ...
      },
      {
        ...
        body: [
          ...
          '普通问题优先使用 query_spdex：把用户问题和已知比赛直接交给服务端，由服务端选择当前正确的专业工具和参数。用户不需要记忆 structure_signal、market_kind 等内部字段。',
        ],
      },
      {
        heading: '服务更新与工具刷新',
        body: [
          'SPdex 会持续更新服务端业务规则、字段解释和意图映射。通常不需要修改 MCP 地址、token、配置文件，也不需要清理客户端缓存；query_spdex 会自动使用当前服务端能力。',
          '服务端会向支持标准通知的客户端声明工具列表已变化。其他客户端在下一次重新连接时会获取最新目录。第三方客户端自己的旧对话仍可能保留创建时的工具快照。',
        ],
        steps: [
          '日常更新后先直接继续提问，不做任何本地处理。',
          '如果客户端明确显示旧工具数量，关闭并重新开启 SPdex 连接器。',
          '如果旧对话仍提示工具不存在，新建对话后复测。',
          '只有以上步骤仍无效时，才按客户端说明清理其本地缓存或重新添加连接。',
        ],
      },
      {
        heading: '接入失败怎么办',
        body: [
          ...
          '如果模型提示工具 not found，并且连接状态列表中没有 SPdex，说明当前项目或对话没有加载 SPdex 连接器。请先在连接器管理中启用 SPdex，再在当前对话的工具选择器中勾选它；重连后新建对话。一般不需要清理全局缓存。',
