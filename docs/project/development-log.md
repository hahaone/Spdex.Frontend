...
- 在 `AI 管理` 导航新增“业务意图字典”，读取权限沿用 `AiOpsView`，候选审核、发布和回滚要求 `AiOpsManage`。
- 页面提供只解析不调用专业工具的路由预演，展示当前结果、候选结果、工具、参数和置信度。
- 新增候选必须填写候选表达、目标白名单意图、真实样本问题、来源和业务说明；单场意图可填写比赛 ID。
- 候选按草稿、已批准、已驳回、待移除管理；已批准状态与当前版本存在实际差异时才允许发布。
- 当前字典页展示固定工具与已发布别名；版本页支持回滚，所有写操作由后端写入 Admin 审计。
