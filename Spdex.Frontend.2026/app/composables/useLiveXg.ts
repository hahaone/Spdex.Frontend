...
export function useLiveXg(items: Ref<LiveXgRef[]>, intervalMs = 30_000) {
  ...
  async function refresh() {
    ...
    const reqItems = items.value
      .filter(i => Number.isFinite(i.eventId) && i.eventId > 0 && i.homeTeamName && i.guestTeamName)
      .map(i => ({ eventId: i.eventId, homeTeamName: i.homeTeamName, guestTeamName: i.guestTeamName }))
