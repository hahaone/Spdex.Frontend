...
function setTeachingDay(day: 'today' | 'yesterday') {
  selectedDay.value = day
  onDateChange(day === 'today' ? '' : formatOffsetDate(-1))
}
  ...
  <div class="match-page">
    ...
    <div class="filter-bar">
      <div class="filter-group date-picker-group">
        <label>日期</label>
        <div class="status-group day-switch">
          <button :class="['status-btn', { active: selectedDay === 'today' }]" @click="setTeachingDay('today')">
            今日
          </button>
          <button :class="['status-btn', { active: selectedDay === 'yesterday' }]" @click="setTeachingDay('yesterday')">
            昨日
          </button>
        </div>
