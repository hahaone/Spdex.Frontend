...
/** 显示用 Home 小计（NBA 反转时取 away 数据） */
const displayHomeSubtotal = computed(() =>
  isNbaReverse.value ? activeWindow.value?.awaySubtotal : activeWindow.value?.homeSubtotal,
)
/** 显示用 Away 小计（NBA 反转时取 home 数据） */
const displayAwaySubtotal = computed(() =>
  isNbaReverse.value ? activeWindow.value?.homeSubtotal : activeWindow.value?.awaySubtotal,
)

/** 显示用 Home 分组（NBA 反转时取 away 数据） */
const displayHomeGroups = computed(() =>
  isNbaReverse.value ? activeWindow.value?.awayGroups : activeWindow.value?.homeGroups,
)
/** 显示用 Away 分组（NBA 反转时取 home 数据） */
const displayAwayGroups = computed(() =>
  isNbaReverse.value ? activeWindow.value?.homeGroups : activeWindow.value?.awayGroups,
)
...
// ── B9: Home/Away 占比（使用显示用小计，NBA 反转时已互换） ──
function homePercent(): string {
  if (!activeWindow.value) return ''
  const ht = displayHomeSubtotal.value?.grandTotalBet ?? 0
  const at = displayAwaySubtotal.value?.grandTotalBet ?? 0
  const total = ht + at
  if (total === 0) return ''
...
/** Handicap 列高亮：LastPrice 仅 1 个成交价位且 > 3000（篮球阈值，原足球为 > 1000） */
    ...
    <template v-else>
      ...
      <div class="match-header">
        ...
        <h2 class="match-title">
          <span class="team-home">{{ displayHomeTeam }}</span>
          <span class="team-vs">vs</span>
          <span class="team-away">{{ displayAwayTeam }}</span>
        </h2>
        <div class="match-meta">
          开赛时间：{{ formatMatchTime(matchInfo?.matchTime ?? '') }}
          &nbsp;|&nbsp; 篮球亚盘 (Basketball Asian Handicap)
      ...
      <div v-if="activeWindow" class="window-content">
        ...
        <!-- ═══ HOME 区域（NBA 反转时显示为客队数据） ═══ -->
        <template v-if="hasGroupData(displayHomeGroups)">
          <div class="side-header side-home">
            <span class="side-label">Home（主队）</span>
            <span class="side-total">
              TotalBet: <b>{{ formatMoney(displayHomeSubtotal?.grandTotalBet ?? 0) }}</b>
              &nbsp; MaxHold: <b>{{ formatMoney(displayHomeSubtotal?.grandMaxHold ?? 0) }}</b>
              &nbsp; TotalHold: <b>{{ formatMoney(displayHomeSubtotal?.grandTotalHold ?? 0) }}</b>
              <!-- B23 -->
              <template v-if="holdBetRatio(displayHomeSubtotal)">
                &nbsp; TotalHold/TotalBet: <b>{{ holdBetRatio(displayHomeSubtotal) }}</b>
              </template>
            </span>
          </div>
          <div class="table-wrapper">
            <table class="hc-table">
              ...
              <tbody>
                <template v-for="group in (displayHomeGroups ?? [])" :key="'hg-' + group.label">
        ...
        <!-- ═══ AWAY 区域（NBA 反转时显示为主队数据） ═══ -->
        <template v-if="hasGroupData(displayAwayGroups)">
          <div class="side-header side-away">
            <span class="side-label">Away（客队）</span>
            <span class="side-total">
              TotalBet: <b>{{ formatMoney(displayAwaySubtotal?.grandTotalBet ?? 0) }}</b>
              &nbsp; MaxHold: <b>{{ formatMoney(displayAwaySubtotal?.grandMaxHold ?? 0) }}</b>
              &nbsp; TotalHold: <b>{{ formatMoney(displayAwaySubtotal?.grandTotalHold ?? 0) }}</b>
              <template v-if="holdBetRatio(displayAwaySubtotal)">
                &nbsp; TotalHold/TotalBet: <b>{{ holdBetRatio(displayAwaySubtotal) }}</b>
              </template>
            </span>
          </div>
          <div class="table-wrapper">
            <table class="hc-table">
              ...
              <tbody>
                <template v-for="group in (displayAwayGroups ?? [])" :key="'ag-' + group.label">
        ...
        <!-- 无数据 -->
        <div v-if="!hasGroupData(displayHomeGroups) && !hasGroupData(displayAwayGroups)" class="no-data">暂无数据</div>
